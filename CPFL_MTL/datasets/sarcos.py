def SARCOS(dataroot):
    train_data = loadmat(os.path.join(dataroot,"sarcos_inv.mat"))["sarcos_inv"].astype(np.float32)
    np.random.shuffle(train_data)
    val_data, train_data = train_data[:4448], train_data[4448:]
    test_data = loadmat(os.path.join(dataroot,"sarcos_inv_test.mat"))["sarcos_inv_test"].astype(
        np.float32
    )
