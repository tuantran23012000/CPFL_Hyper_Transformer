...
class HyperSegnetMlp(nn.Module):
    def __init__(
        self,
        preference_dim=3,
...
class HyperSegnetTrans(nn.Module):
    def __init__(
        self,
        preference_dim=3,
        preference_embedding_dim=32,
        ...
        self.num_in = preference_embedding_dim + chunk_embedding_dim
        for i in range(preference_embedding_dim + chunk_embedding_dim):
            setattr(self, f"embedding_layer_{i}", nn.Sequential(nn.Linear(1, hidden_dim),nn.ReLU(inplace=True)))
        self.attention = nn.MultiheadAttention(embed_dim=hidden_dim, num_heads=1)
        self.ffn1 = nn.Linear(hidden_dim,hidden_dim)
        self.ffn2 = nn.Linear(hidden_dim,hidden_dim)
    ...
    def forward(self, preference):
        ...
        for chunk_id in range(self.num_chunks):
            ...
            for i in range(self.num_in):
                x = getattr(self, f"embedding_layer_{i}")(
                    input_embedding[:,i].unsqueeze(0)
                )
                tmp.append(x)
            x = torch.stack(tmp)
            x_ = x
